					...
					struct drm_bridge_state *old_bridge_state)
{
...
#ifdef VERBOSE
	printk(KERN_ERR "DSI_BRIDGE: %s: 12\n", __func__);
#endif
	...
	#ifdef VERBOSE
	printk(KERN_ERR "DSI_BRIDGE: %s: 13\n", __func__);
	#endif
	regmap_write(ctx->regmap, REG_VID_CHA_TEST_PATTERN, 0x00);
#else
	#ifdef VERBOSE
	printk(KERN_ERR "DSI_BRIDGE: %s: 14\n", __func__);
	#endif
...
#ifdef VERBOSE
		printk(KERN_ERR "DSI_BRIDGE: %s: 15\n", __func__);
#endif
		return;
	}
...
#ifdef VERBOSE
		printk(KERN_ERR "DSI_BRIDGE: %s: 16\n", __func__);
#endif
}
